    ...
    if (target.closest(`.${styles.tableIcon}`)) {
      ...
      if (rowKey) {
        ...
        if (showSuccessMessage) {
          showSuccessMessage(getNewAlertMessage(rowKey, MESSAGE.delete_success));
        }
      }
    }
  };
      ...
      {!dataTable || dataTable.length === 0 ? (
        ...
        <div className={styles.tableContainer} onClick={handleTableClick}>
          <Table dataSource={dataTable} pagination={false} size={'middle'} rowKey="id">
            <Column title={() => <p className={styles.columnTitle}>{MODAL_BRIEFCASE_TEXT.name}</p>} dataIndex="name" key="name" className={styles.tableField} />
            <Column title={() => <p className={styles.columnTitle}>{MODAL_BRIEFCASE_TEXT.price}</p>} dataIndex="price" key="price" className={styles.tableField} />
            <Column title={() => <p className={styles.columnTitle}>{MODAL_BRIEFCASE_TEXT.quantity}</p>} dataIndex="quantity" key="quantity" className={styles.tableField} />
            <Column
            ...
            <Column render={() => <CloseSquareOutlined className={styles.tableIcon} />} />
          </Table>
        </div>
      )}
    </>
